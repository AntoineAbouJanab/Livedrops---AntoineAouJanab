...
async function handleMessage({ message, email }) {
  ...
  // order_status → function call + concise answer
  ...
  // complaint → LLM prompt (fallback to static)
  ...
  // chitchat → LLM prompt (fallback to static)
  if (intent === 'chitchat') {
    try {
      const prompt = buildPrompt('chitchat', { message });
      const out = await callLLMMeasured(prompt);
  ...
  // off_topic → LLM prompt (fallback to static)
  if (intent === 'off_topic') {
    try {
  ...
  // violation → LLM prompt (fallback to static)
  if (intent === 'violation') {
    try {
  ...
  // product_search → function call (+ optional LLM summarization on success)
  ...
  // policy_question → ground-truth first, fallback to LLM
  ...
  if (intent === 'complaint') {
    res.reply = "I'm really sorry about this experience. I can help fix it right away. Could you share your order ID (last 10+ chars) and a brief description of the issue? If you prefer, I can also open a support ticket for follow‑up.";
    return finalizeResponse();
  }

  // chitchat → brief and redirect to support topics
  if (intent === 'chitchat') {
    res.reply = 'Hi! How can I help with your order, products, or a policy today?';
    return finalizeResponse();
  }
