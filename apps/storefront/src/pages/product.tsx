  ...
  return (
    <div className="grid md:grid-cols-2 gap-6">
      ...
      <div className="flex flex-col gap-3">
        ...
        <div className={product.stockQty>0? 'text-emerald-400':'text-rose-400'}>
          {product.stockQty>0? `In stock (${product.stockQty})` : 'Out of stock'}
        </div>
        <p className="text-slate-600">{product.description ?? 'Great product.'}</p>
        <button className="bg-cyan-600 text-white rounded px-4 py-2 w-fit shadow-[0_0_20px] shadow-cyan-500/20" onClick={() => add({ id: product.id, title: product.title, price: product.price, image: product.image })}>Add to Cart</button>
        <div>
